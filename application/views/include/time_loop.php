	...
	<?php else: ?>
		...
		<?php foreach ($times as $time): ?>
			...
			<div class="time_group pt-1 pl-0">
				<div class="btn-group w-100">
				    ...
				    <label data-val="<?php echo html_escape($time_view) ?>" data-id="<?php echo html_escape($time['id']) ?>" class="btn btn-outline-success btn-sm time_btn <?php if($session->enable_group_booking == 1 && $slot_count == $session->group_booking_slot){echo 'disabled';} ?> <?php if($session->enable_group_booking == 0 && $check == true){echo 'disabled';} ?>">
