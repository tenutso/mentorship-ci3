<script type="text/javascript">
    ...
    <?php if (isset($flutterwave_type) && $flutterwave_type == 'admin'): ?>                                          
        document.addEventListener("DOMContentLoaded", (event) => {
            ...
            const email = '<?php echo html_escape($customer_email) ?>';
            const phone = '<?php echo html_escape($customer_phone) ?>';
            ...
            getpaidSetup({
                ...
                callback: function(response) {
                    ...
                    window.location.href='<?php echo base_url('admin/payment/payment_success/'.html_escape($booking->id).'/flutterwave') ?>';
    ...
    <?php else: ?>
        ...
        document.addEventListener("DOMContentLoaded", (event) => {
            ...
            document.getElementById("flutterwave_payment").addEventListener("click", (e) => {
            e.preventDefault();
            const PBFKey = '<?php echo html_escape($public_key) ?>'; // paste in the public key from your dashboard here
            const txRef = ''+Math.floor((Math.random() * 1000000000) + 1); //Generate a random id for the transaction reference
            const email = '<?php echo html_escape($email) ?>';
            const phone = '<?php echo html_escape($phone) ?>';
            const amount = '<?php echo html_escape($price) ?>';
            ...
            getpaidSetup({
                ...
                amount: amount,
                customer_phone: phone,
                currency: "<?php echo html_escape($currency_code) ?>",  // Select the currency. leaving it empty defaults to NGN
                txref: txRef, // Pass your UNIQUE TRANSACTION REFERENCE HERE.
