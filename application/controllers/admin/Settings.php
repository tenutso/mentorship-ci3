...
class Settings extends Home_Controller
{
    ...
    public function update()
    {
        ...
        if ($_POST) {
            ...
            if (!empty($this->input->post('enable_multilingual', true))) {
                $enable_multilingual = $this->input->post('enable_multilingual', true);
            } else {
                $enable_multilingual = 0;
            }

            if (!empty($this->input->post('enable_registration', true))) {
                $enable_registration = $this->input->post('enable_registration', true);
            } else {
                $enable_registration = 0;
            }

            if (!empty($this->input->post('enable_email_verify', true))) {
                $enable_email_verify = $this->input->post('enable_email_verify', true);
            } else {
                $enable_email_verify = 0;
            }

            if (!empty($this->input->post('enable_sms_verify', true))) {
                $enable_sms_verify = $this->input->post('enable_sms_verify', true);
            } else {
                $enable_sms_verify = 0;
            }

            if (!empty($this->input->post('enable_wildapricot_sso', true))) {
                $enable_wildapricot_sso = $this->input->post('enable_wildapricot_sso', true);
            } else {
                $enable_wildapricot_sso = 0;
            }

            if (!empty($this->input->post('enable_mentor_auto_approve', true))) {
                $enable_mentor_auto_approve = $this->input->post('enable_mentor_auto_approve', true);
            } else {
                $enable_mentor_auto_approve = 0;
            }

            if (!empty($this->input->post('enable_sms', true))) {
                $enable_sms = $this->input->post('enable_sms', true);
            } else {
                $enable_sms = 0;
            }


            if (!empty($this->input->post('enable_whatsapp_msg', true))) {
                $enable_whatsapp_msg = $this->input->post('enable_whatsapp_msg', true);
            } else {
                $enable_whatsapp_msg = 0;
            }

            if (!empty($this->input->post('enable_captcha', true))) {
                $enable_captcha = $this->input->post('enable_captcha', true);
            } else {
                $enable_captcha = 0;
            }

            if (!empty($this->input->post('enable_payment', true))) {
                $enable_payment = $this->input->post('enable_payment', true);
            } else {
                $enable_payment = 0;
            }

            if (!empty($this->input->post('enable_blog', true))) {
                $enable_blog = $this->input->post('enable_blog', true);
            } else {
                $enable_blog = 0;
            }

            if (!empty($this->input->post('enable_faq', true))) {
                $enable_faq = $this->input->post('enable_faq', true);
            } else {
                $enable_faq = 0;
            }

            if (!empty($this->input->post('enable_users', true))) {
                $enable_users = $this->input->post('enable_users', true);
            } else {
                $enable_users = 0;
            }

            if (!empty($this->input->post('enable_workflow', true))) {
                $enable_workflow = $this->input->post('enable_workflow', true);
            } else {
                $enable_workflow = 0;
            }

            if (!empty($this->input->post('enable_feature', true))) {
                $enable_feature = $this->input->post('enable_feature', true);
            } else {
                $enable_feature = 0;
            }

            if (!empty($this->input->post('enable_frontend', true))) {
                $enable_frontend = $this->input->post('enable_frontend', true);
            } else {
                $enable_frontend = 0;
            }

            if (!empty($this->input->post('enable_lifetime', true))) {
                $enable_lifetime = $this->input->post('enable_lifetime', true);
            } else {
                $enable_lifetime = 0;
            }

            if (!empty($this->input->post('enable_coupon', true))) {
                $enable_coupon = $this->input->post('enable_coupon', true);
            } else {
                $enable_coupon = 0;
            }

            if (!empty($this->input->post('enable_animation', true))) {
                $enable_animation = $this->input->post('enable_animation', true);
            } else {
                $enable_animation = 0;
            }

            if (!empty($this->input->post('enable_kyc', true))) {
                $enable_kyc = $this->input->post('enable_kyc', true);
            } else {
                $enable_kyc = 0;
            }

            if (!empty($this->input->post('enable_pwa', true))) {
                $enable_pwa = $this->input->post('enable_pwa', true);
            } else {
                $enable_pwa = 0;
            }


            $google_analytics = $this->security->xss_clean($this->input->post('google_analytics', true));
            ...
            $data = array(
                ...
                'enable_wildapricot_sso' => $enable_wildapricot_sso,
                ...
                'wildapricot_button_text' => $this->input->post('wildapricot_button_text', true),
                'wildapricot_domain' => $this->input->post('wildapricot_domain', true),
                'wildapricot_account_id' => $this->input->post('wildapricot_account_id', true),
                'wildapricot_client_id' => $this->input->post('wildapricot_client_id', true),
                'wildapricot_client_secret' => $this->input->post('wildapricot_client_secret', true),
