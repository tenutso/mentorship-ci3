...
class Blog extends Home_Controller {
    ...
    public function index()
    {
        ...
        if (is_admin()){
            $uid = 0; 
            $data['blogs'] = $this->admin_model->get_admin_blogs();
        }else{
            $uid = $this->business->uid; $type = $_GET['type'];
            $data['blogs'] = $this->admin_model->select_by_company('blogs');
        } 

        
        $data['categories'] = $this->admin_model->get_categories($uid, 'blog', 'all');
