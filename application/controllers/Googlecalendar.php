...
class Googlecalendar extends CI_Controller {
    ...
    public function sync() { 
        if (!$this->isLogin()) {
            ...
            $booking = $this->common_model->get_by_md5_id($booking_id, 'session_booking');
            ...
            $start = $booking->date.'T'.$start_time.':00';
            $end = $booking->date.'T'.$end_time.':00';
            ...
            $event = array(
                ...
                'start'       => $start,
                'end'       => $end,
                'description' => $session->name.' booked',
            );
            ...
            if ($this->session->userdata('role') == 'mentee') {
                ...
                $this->admin_model->update($cdata, $booking->id, 'session_booking');
                $this->session->set_flashdata('msg', 'Sync Successfully'); 

                redirect(base_url('admin/sessions/booking_details/'.$booking->booking_number));
            }
    ...
    public function login() {  
        ...
        if ($this->session->userdata('is_authenticate_user') == TRUE) {
            ...
            $data['title'] = "Google Plus Login - Cosmo";
    ...
    public function getCalendar() { 
        ...
        echo html_escape($data['calendar']);
        }
    }
