...
(function ($) {
  ...
  var msg_your_accoun_verified_successfully = $(
    ".msg_your_accoun_verified_successfully"
  ).val();
  var msg_verify_code_is_not_matched = $(
    ".msg_verify_code_is_not_matched"
  ).val();
  ...
  $(document).ready(function () {
    ...
    $(".carousel-4").owlCarousel({
      ...
      nav: true,
      navText: [
        "<i class='bi bi-arrow-left owlstyle-2'></i>",
        "<i class='bi bi-arrow-right owlstyle-2'></i>",
      ],
      responsiveClass: true,
      dots: false,
      responsive: {
    ...
    $.post(
      ...
      function (json) {
        if (json.st == 1) {
          ...
          $(".error").html(
            '<i class="fa fa-exclamation-circle"></i> ' + msg_wrong_access
          );
          $("#login_pass").val("");
        } else if (json.st == 2) {
          $(".signin_btn").prop("disabled", false);
          $(".signin_btn").html("Sign In");
          $(".error").html(
            '<i class="icon-exclamation"></i> ' + msg_not_active
          );
        } else if (json.st == 3) {
          $(".signin_btn").prop("disabled", false);
          $(".signin_btn").html("Sign In");
          $(".error").html(
            '<i class="fa fa-ban"></i> ' + msg_account_suspend_msg
          );
        } else if (json.st == 4) {
          $(".signin_btn").prop("disabled", false);
          $(".signin_btn").html("Sign In");
          $(".error").html(
            '<i class="fa exclamation-circle"></i> ' + msg_email_not_verified
          );
        }
      },
      "json"
  ...
  $(document).on("submit", "#register_form", function () {
    ...
    console.log($("#register_form").attr("action"));
    $.post(
        ...
        if (json.st == 1) {
          ...
          $(".success").html(
            `<i class="fa fa-check-circle"></i> ` +
              msg_registared_successfully +
              ` <br> 
                      <span class="spinner-border spinner-border-sm"></span> ` +
              msg_preparing_environment
          );
          ...
          $(".error").html(
            '<i class="icon-exclamation"></i> ' + msg_email_exist
          );
          $("html, body").animate({ scrollTop: 25 }, "slow");
        } else if (json.st == 4) {
          $(".register_button").prop("disabled", false);
          $(".register_button").html("Register");
          $(".error").html(
            '<i class="icon-exclamation"></i> ' + msg_phone_exist
          );
          $("html, body").animate({ scrollTop: 25 }, "slow");
        } else if (json.st == 3) {
          $(".register_button").prop("disabled", false);
          $(".register_button").html("Register");
          $(".error").html(
            '<i class="icon-exclamation"></i> ' + msg_recaptcha_is_required
          );
          $("html, body").animate({ scrollTop: 25 }, "slow");
        } else {
          $(".register_button").prop("disabled", false);
  ...
  $(document).on("submit", "#verify_from", function () {
    ...
    $.post(
      $("#verify_from").attr("action"),
      $("#verify_from").serialize(),
      function (json) {
        if (json.st == 1) {
          swal(
            {
          ...
          $(".verify_btn").html(
            '<i class="ficon flaticon-check"></i> Verify Code'
          );
          swal({
            title: msg_error,
  ...
  $(document).on("click", ".wa-oauth-login", function () {
    location.href = "/wa/auth/oauth";
  });
  ...
  $(document).on("submit", ".mentor_search_form", function () {
    $.get(
      $(".mentor_search_form").attr("action"),
      $(".mentor_search_form").serialize(),
      function (json) {
        if (json.st == 1) {
  ...
  $(document).on("change", ".register_category", function () {
    ...
    if (Id != "") {
      ...
      $.post(
        url,
        { data: "value", csrf_test_name: csrf_token, id: Id },
        function (data) {
  ...
  $(document).on("click", ".mentee_register_btn", function () {
    ...
    $(".mentee_register_btn").addClass("btn-primary");
    $(".mentee_register_btn").removeClass("border-1");
    $(".mentor_register_btn").addClass("border-1");
    $(".mentor_register_btn").removeClass("btn-primary");
  ...
  $(document).on("click", ".mentor_register_btn", function () {
    ...
    $(".mentee_register_btn").removeClass("btn-primary");
    $(".mentee_register_btn").addClass("border-1");
    $(".mentor_register_btn").addClass("btn-primary");
